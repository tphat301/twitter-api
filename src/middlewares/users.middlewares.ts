  ...
  checkSchema(
    {
      email: {
        ...
        isLength: {
          ...
          errorMessage: USERS_MESSAGES.EMAIL_LENGTH_MUST_BE_FROM_6_TO_100
        },
        custom: {
...
export const verifiedUserValidator = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (verify !== UserVerifyStatus.Verified) {
    // Lỗi không đủ quyền truy cập do chưa xác thực
    return next(
      new ErrorWithStatus({
    ...
    // CHÚ Ý:
    // Nếu không phải là async func thì có thể dùng throw new err
    // Còn nếu dùng async func thì dùng next(new Err) và next(new Err) dùng cho cả đồng bộ và bất đồng bộ req func handler
  }
  next()
}
